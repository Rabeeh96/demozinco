...
class TransactionPageLoan extends StatefulWidget {

  String? date,loanType,type,from_account_id,description,from_accountName,total_intrest,outStandingAmount,to_account_id,to_accountName,amount,id;
  bool? isDetail, fromAccounts, isProcessingFee, closeLoan,isInterest;
  ...
  @override
  State<TransactionPageLoan> createState() => _TransactionPageLoanState();
}

class _TransactionPageLoanState extends State<TransactionPageLoan> with SingleTickerProviderStateMixin {
  var userInput = '';
  var finalAmount;
...
//asset
                                    ...
                                    children: [
                                      Padding(
                                        ...
                                        child: accountListShownFrom[index].account_type == "2"
                                            ? SvgPicture.asset(
                                                color: selectedFromAccountID == accountListShownFrom[index].id ? Colors.white : Colors.black,
  ...
  void initState() {
    ...
    if (widget.type == "Edit") {
      ...
      markAsZakah = ValueNotifier(false);
    } else {
      ...
      markAsInterest = ValueNotifier(widget.isInterest!);
      markAsZakah = ValueNotifier(false);
      finalAmount = roundStringWithOnlyDigit("0");
      if (widget.loanType == "0") {
        emiAmount = widget.amount!;
      }

      if(widget.closeLoan!){
        print("____________________________________________________________ ${widget.outStandingAmount}");
        finalAmount = roundStringWithOnlyDigit(widget.outStandingAmount??'0');
      }
                                        ...
                                        children: [
                                          ...
                                          Row(
                                            ...
                                            children: [
                                              Text(
                                                roundStringWith(finalAmount),
                                                style: customisedStyle(context, Color(0xff7B7B7B), FontWeight.w500, 16.0),
                                                overflow: TextOverflow.clip,
                                              ),
                                              Text(
                                                " " + countryCurrencyCode,
                                                style: customisedStyle(context, Color(0xff6E88A6), FontWeight.normal, 12.0),
                                                overflow: TextOverflow.clip,
                                              ),
                                            ],
                                          ),
                    ...
                    Padding(
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          IconButton(
                              onPressed: () {
                                accountBtmSheet(context, selectedFromAccountID);
                              },
                            ...
                            if (index == 14) {
                              return RoundCustomButton(
                                  ...
                                  buttonTapped: () {
                                    ...
                                    if (widget.isProcessingFee == false) {
                                      setState(() {
                                ...
                                builder: (BuildContext ctx, fromDateNewValue, _) {
                                  return GestureDetector(
                                    ...
                                    child: Container(
                                      ...
                                      width: mWidth * .35,
                                      decoration:
                                          BoxDecoration(borderRadius: BorderRadius.circular(30), border: Border.all(color: Color(0xffD6E0F6))),
                                      child: Text(
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      Padding(
                        ...
                        child: IconButton(
                            onPressed: () {
                              print(widget.loanType);



                              var amount = double.parse(finalAmount);
                              if(checkIsEmpty(amount)) {
                                if (selectedFromAccountID == "" || selectedToAccountID == "") {
                                  msgBtmDialogueFunction(context: context, textMsg: "Select account details");
                                } else {

                                  if(double.parse(finalAmount) <0){
                                    msgBtmDialogueFunction(context: context, textMsg: "Amount must be greater than 0");
                                  }
                                  else{
                                    ...
                                    if(widget.closeLoan!){
                                      ...
                                      var minimum = double.parse(widget.outStandingAmount!)-double.parse(widget.total_intrest!);
                                      if(minimum <0){
                                        minimum = 0;
                                      }

                                      bool isValidAmount=  isDoubleBetween(value: double.parse(finalAmount),minValue: minimum,maxValue: double.parse(widget.outStandingAmount!),);

                                      if(isValidAmount){
                                        CreateExpenseTransaction();
                                      }
                                      else{
                                        msgBtmDialogueFunction(context: context, textMsg: "The final sum is in the range of ${roundStringWith(minimum.toString())} and ${widget.outStandingAmount!} Make sure it is inside this range, please.");
                                      }
                                    }
                                    else{
                                      CreateExpenseTransaction();
  ...
  bool isDoubleBetween({required double value,required double minValue,required double maxValue}) {
    print("value    $value");
    ...
    if (netWork) {
      ...
      try {
        ...
        Map data = {
          ...
          "amount": roundStringWithOnlyDigit(amount),
          "description": notesController.text,
          "finance_type": 1,
          "asset_master_id": "",
          "is_asset": false,
          "is_reminder": false,
          "reminder_date": apiDateFormat.format(fromDateNotifier.value),
          "late_fee": roundStringWithOnlyDigit(lateFee),
          'is_close_loan': widget.closeLoan
        };

        print(data);
        print("lateFee $lateFee  amount $amount  $emiAmount emiAmount ");
        var body = json.encode(data);
        var response = await http.post(Uri.parse(url),
            headers: {
              "Content-Type": "application/json",
  ...
  equalPressed() {
    try {
      ...
      if (isValid) {
        ...
        emiAmount = "${double.parse(finalAmount) + double.parse(widget.amount!)}";
        print("_____________________$emiAmount");
      }
    } catch (e) {
      print(e.toString());
      emiAmount = widget.amount;
