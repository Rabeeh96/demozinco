...
class _InterestListState extends State<InterestList> {
  ...
  listInterestFunction() async {
    var netWork = await checkNetwork();

    if (netWork) {
      if (!mounted) return;
      return BlocProvider.of<ZakathInterestListBloc>(context).add(FetchInterestZakathListEvent(filter: "I"));
    } else {
      if (!mounted) return;
                          ...
                          return GestureDetector(
                            onTap: () async {
                              final result = await Navigator.of(context).push(MaterialPageRoute(
                                  builder: (context) => InterestDetailList(
