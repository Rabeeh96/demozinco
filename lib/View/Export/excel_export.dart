...
Future<void> convertToExcelFromToAmountAlso({required data,required heading,required date}) async {
  ...
  final headerStyleFromAmount = sheet.getRangeByName('D3:D${data.length+2}').cellStyle;
  headerStyleFromAmount.hAlign = HAlignType.right;
  final headerStyles = sheet.getRangeByName('E3:E${data.length+2}').cellStyle;
  ...
  final headerStyle = sheet.getRangeByName('A3:F3').cellStyle;
  ...
  sheet.getRangeByIndex(1, 4 + 1).columnWidth = 15.00;
  sheet.getRangeByIndex(1, 5 + 1).columnWidth = 40.00;
  ...
  sheet.getRangeByIndex(1, 5 + 1).cellStyle.fontName = "Lexend";
