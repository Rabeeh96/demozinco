...
Future<void> convertToExcel({required data,required heading,required date,required balance}) async {
  final Workbook workbook = Workbook();
  final Worksheet sheet = workbook.worksheets[0];

  final Range range1 = sheet.getRangeByName('A1:D1');
  range1.merge();
  sheet.getRangeByName('A1').setText(heading +"   "+balance);
  ...
  final headerStyles = sheet.getRangeByName('C3:C${data.length+2}').cellStyle;
  ...
  final headerStyle = sheet.getRangeByName('A3:D3').cellStyle;
  ...
  sheet.getRangeByIndex(1, 1 + 1).columnWidth = 50.00;

  sheet.getRangeByIndex(1, 2 + 1).columnWidth = 15.00;

  sheet.getRangeByIndex(1, 0 + 1).columnWidth = 15.00;

  sheet.getRangeByIndex(1, 3 + 1).columnWidth = 40.00;
